...
class Formbuilder {
    ...
    private $config = array(/* Config array - can be overrided by passing in array in ini() */
        ...
        'default_form_control_class' => 'col-md-6',
...
//        }
    ...
    private function _build_input($include_pre_post = true) {
        ...
        if ($this->func == 'form_combine') {
            ...
            switch ($this->func) {
                 ...
                 */
                case 'form_json':
                    $input_html_string = $this->_recursive_build_json((array) json_decode(html_entity_decode($this->elm_options['value'])));
                    break;
                case 'form_button':
                case 'form_anchor':
                case 'form_a':
                ...
                case 'form_editor':
                    ...
                    break;
                case 'form_autocomplete': //menggunakan plugin jquery easyautocomplete
                    $add='';
                    if(isset($this->elm_options['add']))
                        $add='Jika data belum ada, klik <a href="'.$this->elm_options['add'].'" target="_blank">tambah data</a>';
                    $input_html_string= form_input($this->elm_options).$add;
                    $input_html_string.='<script type="text/javascript">
                                var options = {
                                        url: function(phrase) {
                                            return "'.$this->elm_options['url'].'/"+phrase;
                                        },
                                        getValue: "value",
                                        list: {
                                                match: {
                                                ...
                                                onClickEvent: function() {
                                                    var value = $("#'.$this->elm_options['id'].'").getSelectedItemData().id;
                                                    $("input[name='.$this->elm_options['completeid'].']").val(value);
                                                    //alert("id = "+$("input[name='.$this->elm_options['completeid'].']").val());
                                                },
                                                onLoadEvent: function() {
                                                    $("input[name='.$this->elm_options['completeid'].']").val("");
                                                    //alert("id = "+$("input[name='.$this->elm_options['completeid'].']").val());
                                                },
                                        }
                                };
                                $("#'.$this->elm_options['id'].'").easyAutocomplete(options);
                            </script>';
                    break;
                case 'form_file':
    ...
    private function _build_help_block() {
        if (!empty($this->elm_options_help)) {
            return '<small class="text-danger">' . $this->elm_options_help . '</small>';
        }
        return '';
    }
