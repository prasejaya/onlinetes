  ...
  */
  class Datatables
  {
    ...
    */
    ...
    /** component function **/
    public function generate_js($controller, $a_kolom, $indexaksi, $customcolumndef='',$export=false){
        
        $kolom='';
        if(is_array($a_kolom)){
            foreach ($a_kolom as $key => $value) {
                $kolom.='{"data": "'.$key.'"},';
            }        
        }
        if($export){
            $jsexport='dom: "Bfrtipl",
                    buttons: [
                           // "copy", "csv", "excel", "pdf", "print",
                            {
                                text: "Print",
                                action: function ( e, dt, button, config ) {
                                window.open("pendaftar/cetakdata", "_blank");
    ...
    public function generate_jsdelete($action){
        $js='function hapusdata(id){
    ...
    public function generate_jsedit($action, $jsupdate){
        $js='function editdata(id){
                $("#modal-title").html("Ubah");
                $("#formupdate").trigger("reset");
                $("#formupdate").attr("action","'.$action.'"); 
                $("#idtableupdate").val(id); 
                '.$jsupdate.'                
                $("#formUpdateModal").modal("show");
                }';
        return $js;
    }
    /** component modal insert **/
    public function generate_jsinsert($action, $jsinsert){
        $js='function insertdata(){
                $("#modal-title").html("Tambah");
                $("#forminsert").trigger("reset");
                $("#forminsert").attr("action","'.$action.'"); 
                '.$jsinsert.'  
    ...
    public function generate_jsfilter($formfilter, $controller=''){
        $js='function filterdata(){';
        $jsurl='';
        foreach ($formfilter as $row) {
            $js.='var '.$row['id'].'= $("#'.$row['id'].'").val(); ';
            $jsurl.='+'.$row['id'].'+"/"';
        }
        
        if($controller=='')
            $controller=base_url($this->ci->uri->segment(1)).'/index/';
        
        $js.='window.location.replace("'.$controller.'"'.$jsurl.');
            }';
        return $js;
    }
